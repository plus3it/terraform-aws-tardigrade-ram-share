...
def get_invite_status(ram, invite_arn):
    invitations = ram.get_resource_share_invitations(
        resourceShareInvitationArns=[invite_arn])['resourceShareInvitations']
...
def accept_pending_invite(ram, invite_arn, timeout=60):
    ...
    sleep = 1
    while (
        get_invite_status(ram, invite_arn) != 'ACCEPTED' and
        time.time() < start + timeout
    ):
        log.info(
            'Invite is still pending, checking again in %s second(s)', sleep)
        time.sleep(sleep)
        sleep *= 2
...
def main(resource_share_arn, profile=None, role_arn=None, region=None):
    ...
    if not invite_arn:
        raise NoPendingInviteException(
            'No pending invite found for resource share ARN: {}'
            .format(resource_share_arn)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resource-share-arn",
        help="ARN of the Resource Share to accept"
    )
    parser.add_argument(
        "--profile", default=None,
        help="Profile to use when setting up the AWS session"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--region", default=None,
        help="Region in which to look for the RAM invite"
    )
